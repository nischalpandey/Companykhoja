...
import { parseSlug, companySlug } from '~/utils/formatter'
...
const rawSlug = computed(() => String((route.params as { slug: string }).slug))
const companyId = computed(() => {
  const s = rawSlug.value
  return /^\d+$/.test(s) ? s : parseSlug(s)
})
...
function shareCompany() {
  if (!company.value) return
  const url = `${window.location.origin}/company/${companySlug(company.value)}`
  if (navigator.share) {
